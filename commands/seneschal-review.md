...
**Goal:** Spawn the configured reviewer personas in parallel, aggregate their findings into one markdown table, write the result to a state file, and post the review to GitHub under the `seneschal-cr[bot]` identity.

Designed for interactive use in a Claude Code session on the operator's local machine. Run it from inside the cloned repo you want to review.
...
4. **WRITE STATE FILE:** always write `.claude/plans/seneschal-review-{pr-number}.md` so `bin/seneschal-post` can read and post it
...
The PR number is required. Run from inside a clone of the repo you want to review — the command resolves the PR and remote from the current working directory's git config.
...
Check for `.claude/plans/seneschal-personas-${PR}.json`. If present, it enumerates the personas configured for this review. The schema is:
...
**If the state file is missing or malformed**, fall back to the six built-in personas:
